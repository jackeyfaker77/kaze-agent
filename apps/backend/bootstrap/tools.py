...
def build_registered_tools(
    ...
    *,
    ...
    from agent.screen_observation import build_screen_observation_service
    ...
    screen_observation = build_screen_observation_service(
        provider=provider, model=config.model, memory=memory_runtime.engine)
    tools.register(ObserveScreenTool(capture=PrimaryScreenCapture(), analyzer=screen_observation),
                   always_on=True, risk="read-only")
